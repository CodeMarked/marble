...
/// Vertical gradient: `top*` at top edge, `bottom*` at bottom edge (interpolated horizontally constant).
inline void appendNdcUnlitGradientScreenRectPx(
    float leftPx,
    float topPx,
    float rightPx,
    float bottomPx,
    ...
    std::uint32_t const base = static_cast<std::uint32_t>(vtx.size());
    auto push = [&](float nx, float ny, float cr, float cg, float cb) {
        marble::render::VulkanRhi::Vertex v{};
        v.px = nx;
        v.py = ny;
        v.pz = 0.f;
    ...
    push(tl.first, tl.second, ctr, ctg, ctb);
    push(tr.first, tr.second, ctr, ctg, ctb);
    push(br.first, br.second, cbr, cbg, cbb);
    push(bl.first, bl.second, cbr, cbg, cbb);
    idx.push_back(base);
    idx.push_back(base + 1);
    idx.push_back(base + 2);
    idx.push_back(base + 2);
